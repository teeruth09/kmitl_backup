...
void loop() {
  int buttonState = debounceAnalogRead(button1);
  Serial.print("check2_F =");
  Serial.println(check2_F);
  ...
  if (digitalRead(BUZZER_PIN) or check)
  {
    ...
    if (sc == false and sc2 == false and check_state_servo == true) 
    {
      ...
      {                                                                                                         //i<=180
        digitalWrite(BUZZER_PIN, HIGH);
        lcd_servo_open();
        myservo.write(45);                                                                                                                       //myservo.write(i)
        myservo2.write(45);
        Serial.println(i);
        delay(10);
      }
    ...
    if (digitalRead(IR_RECEIVER_PIN2) == HIGH and microphone_check > 0  )
    {
    ...
    if (buttonState == LOW and microphone_check > 0)
    {
      digitalWrite(led_green, !digitalRead(led_green));
      digitalWrite(led_yellow, !digitalRead(led_yellow));
      digitalWrite(led_red, !digitalRead(led_red));
    ...
    if (sc == true and sc2 == true and digitalRead(IR_RECEIVER_PIN))
    {
      ...
      {                                                                                                                          //i>60
        digitalWrite(BUZZER_PIN, HIGH);
        lcd_servo_close();
        myservo.write(180);                                                                                                                                      //myservo.write(i)
        myservo2.write(180);
        Serial.println(i);
        delay(10);
      }
